    ...
    <div class="max-w-md mx-auto p-6">
      <form @submit.prevent="submit">
        <div class="bg-white rounded shadow p-8">
          <input v-model="form.name" type="text" placeholder="Role name" class="w-full border-b border-gray-300 focus:border-teal-600 outline-none text-lg mb-2" :class="{'border-red-500': errors.name}" />
          <div v-if="errors.name" class="text-red-500 text-xs mb-2">{{ errors.name }}</div>
          <div class="mb-4">
            <label><input type="checkbox" value="pos" v-model="form.guards" /> POS</label>
            <label class="ml-6"><input type="checkbox" value="backoffice" v-model="form.guards" /> Back office</label>
          </div>
          <div v-if="form.guards.includes('pos')" class="mb-4">
            ...
            <div v-for="perm in permissions.pos" :key="perm">
              <label><input type="checkbox" :value="perm" v-model="form.permissions.pos" /> {{ perm }}</label>
            </div>
          </div>
          <div v-if="form.guards.includes('backoffice')" class="mb-4">
            ...
            <div v-for="perm in permissions.backoffice" :key="perm">
              <label><input type="checkbox" :value="perm" v-model="form.permissions.backoffice" /> {{ perm }}</label>
            </div>
          </div>
        ...
        <div class="flex justify-end gap-2 mt-4">
          ...
          <button type="submit" class="px-6 py-2 rounded bg-green-500 text-white font-semibold hover:bg-green-600" :disabled="saving">{{ saving ? 'Saving...' : 'SAVE' }}</button>
        </div>
      </form>
    </div>
  </AppLayout>
...
const page = usePage();
const permissions = page.props.permissions || { pos: [], backoffice: [] };
...
const submit = () => {
  ...
  router.post('/employers/roles', {
    ...
    onSuccess: () => {
      Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: page.props.flash?.success || 'Role created!', showConfirmButton: false, timer: 2000 });
      saving.value = false;
    },
    onError: () => {
      Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: page.props.flash?.error || 'Failed to create role', showConfirmButton: false, timer: 3000 });
      saving.value = false;
    },
